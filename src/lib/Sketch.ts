import { P5Sketch } from "@netalondon/svelte-p5";
...
export class Sketch extends P5Sketch {
  ...
  private getColor(u: number) {
    ...
    let d = this.p5.createVector(this.state[9], this.state[10], this.state[11]);
  ...
  public draw(): void {
    ...
    let flexes: number[] = [];
    for (let i = 0; i < this.rectNum; i++) {
      let noise = this.p5.noise(this.rectOffsets[i], this.t * this.noiseYSpeed);
